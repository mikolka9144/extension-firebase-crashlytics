...
2. Patch the "hxcpp" compiler
    ...
    (which is required for the crashlytics on Android to work)
...
3. Patch "lime" library
    ...
    For iOS, you can disable the included patched with ``NO_FIREBASE_GRADLE_PATCH``.

    Useful if you want to retain your modifications to the gradle/Xcode files.
    (this also disables ``lime_disable_assets_version`` for you to decide if you want to use it)
4. Add ``google-services.json\plist`` file

    You can do so with 
    ```xml 
    <template path="assets/google-services.json" rename="app/google-services.json" if="android"/>

	<template path="assets/GoogleService-Info.plist" rename="GoogleService-Info.plist" if="ios"/>
    <!-- and/or -->
    <template path="assets/GoogleService-Info.plist" rename="../GoogleService-Info.plist" if="ios"/>
    ```
5. Set the minimum SDK versions on Android/iOS

    Firebase crashlytics is not available on devices older than Android 6.0 and iOS 15, so we have to bump it.

    You can accomplish this by adding 
    ```xml	
    <config type="android" minimum-sdk-version="26" />
	<config type="ios" deployment="15.0"/>
    ``` 
...
6. Add the symbol upload script
    ...
    Here's the symbol upload script for iOS version.

    ```bash
    # Replace "export/ios/"  with the path where the android build is stored

    chmod +x ./export/ios/upload-symbols
    ./export/ios/upload-symbols -gsp ./assets/GoogleService-Info.plist -p ios  ./export/ios/build/Release-iphoneos/Tilemap.app.dSYM
    ```
...
Any ANR and native crashes will be sent over automatically.
